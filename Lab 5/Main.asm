...
###########################################################################################################################################################################################
			...
			move $a1,$v1			#Moves returned value into $a1  for use In atoi
		       ...
		       #--->> THIS LINE WOULD CHECK LENGTH OF STRING TO NOT EXEED 5 DIGITITS SINCE 16-bit-INT !> 65535
		       move $t9,$v1			#moves returned value into $t9  for quiq check
		       bgt  $t9,5,ERROR1			#will check if nuber will exid 16 bit mx chars  <<-----------#
